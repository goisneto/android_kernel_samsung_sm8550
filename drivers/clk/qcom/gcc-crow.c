...
static struct clk_vdd_class *gcc_crow_regulators[] = {
	&vdd_cx,
	&vdd_mxa,
};
...
static struct clk_rcg2 gcc_pcie_0_aux_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = gcc_crow_regulators,
		.num_vdd_classes = ARRAY_SIZE(gcc_crow_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 19200000},
	},
...
static struct clk_regmap *gcc_crow_clocks[] = {
...
static const struct qcom_reset_map gcc_crow_resets[] = {
	...
	[GCC_PCIE_1_LINK_DOWN_BCR] = { 0x2f014 },
	[GCC_PCIE_1_NOCSR_COM_PHY_BCR] = { 0x2f020 },
	[GCC_PCIE_1_PHY_BCR] = { 0x2f01c },
	[GCC_PCIE_1_PHY_NOCSR_COM_PHY_BCR] = { 0x2f028 },
	...
	[GCC_VIDEO_AXI1_CLK_ARES] = { 0x3201c, 2 },
...
static const struct regmap_config gcc_crow_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc gcc_crow_desc = {
	.config = &gcc_crow_regmap_config,
	.clks = gcc_crow_clocks,
	.num_clks = ARRAY_SIZE(gcc_crow_clocks),
	.resets = gcc_crow_resets,
	.num_resets = ARRAY_SIZE(gcc_crow_resets),
	.clk_regulators = gcc_crow_regulators,
	.num_clk_regulators = ARRAY_SIZE(gcc_crow_regulators),
};

static const struct of_device_id gcc_crow_match_table[] = {
	{ .compatible = "qcom,crow-gcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_crow_match_table);

static int gcc_crow_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_crow_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &gcc_crow_desc, regmap);
	if (ret) {
...
static void gcc_crow_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_crow_desc);
}

static struct platform_driver gcc_crow_driver = {
	.probe = gcc_crow_probe,
	.driver = {
		.name = "gcc-crow",
		.of_match_table = gcc_crow_match_table,
		.sync_state = gcc_crow_sync_state,
	},
};

static int __init gcc_crow_init(void)
{
	return platform_driver_register(&gcc_crow_driver);
}
subsys_initcall(gcc_crow_init);

static void __exit gcc_crow_exit(void)
{
	platform_driver_unregister(&gcc_crow_driver);
}
module_exit(gcc_crow_exit);

MODULE_DESCRIPTION("QTI GCC CROW Driver");
